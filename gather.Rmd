...
```{r}
full_dataset %>%
  select(team, valuation, operating_income, revenue, debt_to_value, gate_receipts, stadium, stadium_pct, income_no_fans, revenue_no_fans) %>%
  arrange(desc(revenue))
```

```{r}
nn=neuralnet(revenue ~ operating_income + gate_receipts + 
                metro_area_pop, data = full_dataset, hidden=c(5, 2),
                linear.output = TRUE)
